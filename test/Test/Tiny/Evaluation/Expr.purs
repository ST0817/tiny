...
spec = describe "expression" do
  ...
  it "variable" do
    -- foo
    -- -> 42
    let scope = singleton "foo" $ IntObj 42
    shouldEqual
      (runEvaluator (evalExpr $ Var "foo") scope)
      (Right $ IntObj 42 /\ scope)

  it "tuple" do
    -- (42 + 53, 42 < 53)
    -- -> (95, true)
    let
      elem1 = BinExpr (IntLit 42) AddOp (IntLit 53)
      elem2 = BinExpr (IntLit 42) LTOp (IntLit 53)
    shouldEqual
      (runEvaluator (evalExpr $ TupleExpr [ elem1, elem2 ]) empty)
      (Right $ TupleObj [ IntObj 95, BoolObj true ] /\ empty)

  it "tuple (single element)" do
    -- (4 + 1) * 5
    -- -> 25
    let
      lhs = TupleExpr [ BinExpr (IntLit 4) AddOp (IntLit 1) ]
      rhs = IntLit 5
    shouldEqual
      (runEvaluator (evalExpr $ BinExpr lhs MulOp rhs) empty)
      (Right $ IntObj 25 /\ empty)
